...
real(dble) parA(12),parU(7),parW(7),parH(6),beta,sigma1(shocksize1,shocksize1),parB(Bsizeexo+1,nfert)
...
contains
	...
	subroutine distance(dist,parameters,weightmat)
		implicit none
		use 'mpif.h'
		...
		if (rank==0) then
			...
			do id=1,SampleSize
				call simhist(SS,outcomes,testoutcomes, choices, xchoices,birthhist,smchoices, smexperience, smAs, smtestoutcomes,gomega3data(:,id),intercepts,parA,parU,gparW,gparH,beta,sigma,a1type,pa1type,gparBmat,solvall,solwall,llmsmat(:,id),id,grho,glambdas,gsigmaetas,gsmpar)
				SScollect(:,:,:,id)=SS
